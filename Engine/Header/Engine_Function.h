...
namespace Engine
{
	...
	template <typename T>
	class CTag_FinderWstring
	{
	public:
		explicit CTag_FinderWstring(const T& pTag) : m_wstrTargetTag(pTag) {}
		~CTag_FinderWstring(void) {}

	public:
		template<typename T>
		_bool		operator()(const T& pair)
		{
			if (m_wstrTargetTag == pair.first)
				return true;
