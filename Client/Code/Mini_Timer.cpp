...
CMini_Timer::CMini_Timer(LPDIRECT3DDEVICE9 pGraphicDev)
	: Engine::CGameObject(pGraphicDev)
{
}

CMini_Timer::CMini_Timer(const CMini_Timer& rhs)
	: Engine::CGameObject(rhs)
{
}

CMini_Timer::~CMini_Timer()
{
}



HRESULT CMini_Timer::Add_Component()
{
...
void CMini_Timer::Set_TexNum(float m_Tex)
{
	m_iTextureIndex = m_Tex;
}

HRESULT CMini_Timer::Ready_GameObject(float pPos_X, int Count)
{
...
_int CMini_Timer::Update_GameObject(const _float& fTimeDelta)
{
...
void CMini_Timer::LateUpdate_GameObject()
{
	__super::LateUpdate_GameObject();
}

void CMini_Timer::Render_GameObject()
{
...
CMini_Timer* CMini_Timer::Create(LPDIRECT3DDEVICE9 pGraphicDev, float pPos_X, int Count)
{
	CMini_Timer* pInstance = new CMini_Timer(pGraphicDev);

	if (FAILED(pInstance->Ready_GameObject(pPos_X, Count)))
	{
		Safe_Release(pInstance);

		MSG_BOX("MINIGame_Timer Create Fail");
		return nullptr;
	}
