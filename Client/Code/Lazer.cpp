...
HRESULT CLazer::Ready_GameObject()
{
	...
	m_pTransformCom->Set_Scale(_vec3(10.f, 3.5f, 10.f));
...
Engine::_int CLazer::Update_GameObject(const _float& fTimeDelta)
{
	...
	if (dynamic_cast<CDyehard*>(m_pGun)->m_bLazer) {
		...
		m_pTransformCom->Set_Pos(vPlayerPos + vPlayerLook / 3);
		//m_pTransformCom2->Update_Component(fTimeDelta);

		m_pTransformCom->Set_Scale(_vec3(0.025f, 0.025f, 0.025f));
		m_pTransformCom->RotateAxis(vPlayerLook, D3DXToRadian(m_SpinAngle3 -= 15));
		m_pTransformCom->Set_Pos(vPlayerPos + vPlayerLook / 5 + vGunMoveRight * m_fGunMoveRight * 0.75 + vGunMoveDown * m_fGunMoveDown * 0.75);

		//D3DXVec3TransformNormal(&vPlayerLook,&vPlayerLook,&m_Mat_Axis);
		m_pTransformCom->RotateAxis(vPlayerLook, D3DXToRadian(150));

		m_pTransformCom->Set_Pos(vPlayerPos + vPlayerLook * 25);
