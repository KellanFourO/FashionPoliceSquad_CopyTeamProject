...
void CMapTool::LateUpdate_Scene()
{
	...
	if (CImGuiManager::GetInstance()->Get_ALLDelete_Check() == true)
		{	Delete_ALL_Cube();	}
}
...
HRESULT CMapTool::Build_OBJ()
{
	if (false == m_BUILD_NOP_MODE && Engine::Get_DIMouseState(DIM_RB)) {
			...
			&& (CursorTemp.z > -1))
		{
			...
			_vec3 ObjSize = { m_fCubesize.m_fX, m_fCubesize.m_fY, m_fCubesize.m_fZ };
			OBJ_TYPE eTypeTemp = CImGuiManager::GetInstance()->Get_OBJType(); //오브젝트 태그랑 다른거임
			pGameObject = CBuild_Obj::Create(m_pGraphicDev, CursorTemp, m_iTextureNum, ObjSize, m_iOBJIndex, eTypeTemp);
...
/*
	Safe_Release ref 카운터가 올라가는데 이미 지웠는데 또 불렀거나, 아직 카운터 올리지않앗는데 불러
*/
				...
				*iter = nullptr;
				...
				if (IndexTemp != -1) {
					...
					for (int i = 0; i < ObjVectorTemp.size(); ++i)
					{
						...
						{
							m_pLayer->Delete_GameObject(OBJECTTAG::BUILD_CUBE, ObjVectorTemp[i], IndexTemp);
							break;
						}
					}
				}
...
HRESULT CMapTool::Delete_ALL_Cube()
{
	//수정해야 함!!!!!!!!!!!!!!!!
	auto& CubeVectorTemp = Engine::Management()->GetInstance()->Get_Scene()->Get_ObjectList(LAYERTAG::ENVIRONMENT, OBJECTTAG::BUILD_CUBE);
	for (int i = 0; i < CubeVectorTemp.size(); ++i)
	{	CubeVectorTemp[i] = nullptr; }

	auto& ObjVectorTemp = Engine::Management()->GetInstance()->Get_Scene()->Get_ObjectList(LAYERTAG::ENVIRONMENT, OBJECTTAG::BUILD_OBJ);
	for (int i = 0; i < ObjVectorTemp.size(); ++i)
	{	ObjVectorTemp[i] = nullptr;	}
	...
	CImGuiManager::GetInstance()->Set_ALLDelete_Check();
	return S_OK;
}
