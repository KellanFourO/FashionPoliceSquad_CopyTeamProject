...
CBossBigDaddy4::CBossBigDaddy4(CMonster& rhs)
	: CMonster(rhs)
{
}

CBossBigDaddy4::~CBossBigDaddy4()
{
}

HRESULT CBossBigDaddy4::Ready_GameObject()
{
...
_int CBossBigDaddy4::Update_GameObject(const _float& fTimeDelta)
{
	...
	_vec3 vStartPos = vBossPos + (-vBossLook * 1.5f) + (vBossLeft * 9.f) + (vBossDown *12.f);

	m_pTransformCom->Set_Pos(vStartPos);

	//m_pTransformCom->Set_Rotate(ROT_Y, vBossAngle);
...
void CBossBigDaddy4::Render_GameObject()
{
...
void CBossBigDaddy4::OnCollisionEnter(CCollider* _pOther)
{
...
void CBossBigDaddy4::OnCollisionStay(CCollider* _pOther)
{
...
void CBossBigDaddy4::OnCollisionExit(CCollider* _pOther)
{
	//__super::OnCollisionStay(_pOther);
}

HRESULT CBossBigDaddy4::Add_Component()
{
...
CBossBigDaddy4* CBossBigDaddy4::Create(LPDIRECT3DDEVICE9 pGraphicDev, _int iIndex)
{
	CBossBigDaddy4* pInstance = new CBossBigDaddy4(pGraphicDev);
	pInstance->m_iIndex = iIndex;

	if (FAILED(pInstance->Ready_GameObject()))
	{
		Safe_Release(pInstance);
		MSG_BOX("BossBigDaddy4 Create Failed");
		return nullptr;
	}
	return pInstance;
}

void CBossBigDaddy4::Free()
{
