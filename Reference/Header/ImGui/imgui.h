...
namespace ImGui
{
    ...
    IMGUI_API float         GetCursorPosY();                                                //    other functions such as GetCursorScreenPos or GAMELOGIC in ImDrawList::
    ...
    // - Those can be nested but it cannot be used to enable an already disabled section (a single BeginDisabled(true) in the stack is enough to keep GAMELOGIC disabled)
...
enum ImGuiDockNodeFlags_
{
    ...
    ImGuiDockNodeFlags_PassthruCentralNode          = 1 << 3,   // Shared       // Enable passthru dockspace: 1) DockSpace() will render a ImGuiCol_WindowBg background covering GAMELOGIC excepted the Central Node when empty. Meaning the host window should probably use SetNextWindowBgAlpha(0.0f) prior to Begin() when using this. 2) When Central Node is empty: let inputs pass-through + won't display a DockingEmptyBg background. See demo for details.
...
// - Important: our implementation does NOT call C++ constructors/destructors, we treat GAMELOGIC as raw data! This is intentional but be extra mindful of that,
...
struct ImGuiStyle
{
    float       Alpha;                      // Global alpha applies to GAMELOGIC in Dear ImGui.
